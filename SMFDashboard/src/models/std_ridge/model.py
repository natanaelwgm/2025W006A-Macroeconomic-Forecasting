...
class _StdRidge(BaseModel):
    """Ridge regression on standardized features; intercept not penalized."""

    def __init__(self, alpha: float = 1.0):
        self.alpha = float(alpha)
        self.coef: List[float] = []  # original space, coef[0]=intercept
        self.mu: List[float] = []
        self.sig: List[float] = []
    ...
    def fit(self, X: List[List[float]], y: List[float]) -> None:
        ...
        # Build normal equations with ridge penalty on non-intercept
        ...
        for i in range(n):
            ...
            for a in range(1, p + 1):
                ...
                for b in range(1, p + 1):
                    G[a][b] += za * zi[b - 1]
        # Add ridge to diagonal (non-intercept)
    ...
    def get_params(self) -> Dict:
        return {"alpha": self.alpha, "coef": self.coef[:], "mu": self.mu[:], "sig": self.sig[:]}
